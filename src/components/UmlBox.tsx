...
export function UmlBox({ data, headingLevel = 'h2', className = '' }: UmlBoxProps) {
  ...
  return (
    <article id={data.id} className={boxClassName} aria-labelledby={titleId}>
      <header className="node-header">
        <span className="node-type">&lt;&lt; {data.stereotype ?? 'component'} &gt;&gt;</span>
        <span className="node-index">{data.id}</span>
      </header>
